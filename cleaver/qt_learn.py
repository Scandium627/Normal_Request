...
class Window(QWidget):
    ...
    def func(self):
        ...
        fd = QFileDialog(self, '文件选择', './', 'excel(*.xls );;python(*.py)')

        # 接收模式和保存模式
        # QFileDialog.AcceptSave
        # QFileDialog.AcceptOpen
        # fd.setAcceptMode(QFileDialog.AcceptSave)  # 必须要有文件后缀名
        fd.setDefaultSuffix('xls')
        fd.setAcceptMode(QFileDialog.AcceptOpen)
        fd.setDefaultSuffix('xls')  # 设置默认文件后缀名
       ...
       # 单个文件被选中
        ...
        # pushButton.setText('打开对话框')
        # btn.resize(100, 30)

        fd.setDefaultSuffix("C:/")
        # self.ui.pushButton.clicked.connect(lambda: fd.show())
        # print(fd.fileSelected())
        print(fd.Detail)
        # fd.fileSelected.connect(lambda: self.xls_file_define())
        self.ui.pushButton.clicked.connect(lambda: self.xls_file_define())
        self.ui.pushButton_3.clicked.connect(lambda: self.close())

    def file_selcet(self):
        self.ui.textBrowser.setText("niubi")
        pass

    def xls_file_define(self):
        res = QFileDialog.getOpenFileNames(self, '文件选择', './','excel(*.xls);;python(*.py)', 'all(*.*)')
        self.ui.textBrowser.setText(res[0][0])
        return res
